...
class BertMapping(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.txt_stru == 'pooling':
            ...
            self.dropout = nn.Dropout(bert_config.hidden_dropout_prob)
            self.mapping = nn.Linear(768, opt.final_dims) # 768 -> 256

    def forward(self, input_ids, attention_mask, token_type_ids, lengths, multi):
        # all_encoder_layers is list 12, insize is [batch_size, seq_length, 768] 
        ...
        if self.txt_stru == 'pooling':
            ...
            output = torch.cat(x, 1) # [batch_size, 1536]
