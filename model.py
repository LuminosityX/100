...
class SAEM(object):
    ...
    def __init__(self, opt):
        ...
        if self.multi:
            ...
            self.txt_multi = bert.BertMultiLayer_word(bert_config)
            self.img_multi = bert.BertMultiAttention_image(bert_config)
        ...
        if self.multi:
            params += list(self.txt_multi.parameters())
            params += list(self.img_multi.parameters())
    ...
    def train_start(self):
        ...
        self.txt_enc.train()
        if self.multi:
            self.img_multi.train()
            self.txt_multi.train()
    ...
    def forward_emb(self, epoch, batch_data, volatile=False):
        ...
        # forward text
        cap_code = self.txt_enc(input_ids, attention_mask, token_type_ids, lengths, self.multi)
        cap_lens = lengths

        # forward image
        img_code = self.img_enc(images, self.multi)

        if self.multi:
            extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
            extended_attention_mask = extended_attention_mask.float()
            extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
            cap_output = self.txt_multi(cap_code, img_code, extended_attention_mask)
            img_output = self.img_multi(img_code, cap_code, extended_attention_mask)
            
            cap_output = torch.mean(cap_output, 1)
            cap_output = F.normalize(cap_output, p=2, dim=1)

            img_output = torch.mean(img_output, 1) # (batch_size, final_dims)
            img_output = F.normalize(img_output, p=2, dim=1)  # (N, C)

            return img_output, cap_output, cap_lens, ids
